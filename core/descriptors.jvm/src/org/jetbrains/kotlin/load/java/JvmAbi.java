...
public final class JvmAbi {
    ...
    public static boolean isPropertyWithBackingFieldInOuterClass(@NotNull PropertyDescriptor propertyDescriptor) {
        ...
        if (isCompanionObjectWithBackingFieldsInOuter(propertyDescriptor.getContainingDeclaration())) return true;
    ...
    public static boolean isJvmFieldPropertyInInterfaceCompanion(@NotNull PropertyDescriptor descriptor) {
        if (!hasJvmFieldAnnotation(descriptor)) return false;

        DeclarationDescriptor container = descriptor.getContainingDeclaration();
        if (!DescriptorUtils.isCompanionObject(container)) return false;

        DeclarationDescriptor grandParent = ((ClassDescriptor) container).getContainingDeclaration();
