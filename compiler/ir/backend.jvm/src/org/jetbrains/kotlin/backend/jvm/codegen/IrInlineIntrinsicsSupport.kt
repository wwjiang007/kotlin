...
import org.jetbrains.kotlin.resolve.jvm.diagnostics.KtErrorsJvm
...
class IrInlineIntrinsicsSupport(
    ...
    override fun checkAnnotatedType(type: IrType) {
        if (type.hasAnnotation(StandardNames.FqNames.extensionFunctionType)) {
            context.ktDiagnosticReporter.at(reportErrorsOn, containingFile).report(KtErrorsJvm.TYPEOF_EXTENSION_FUNCTION_TYPE)
        } else if (type.annotations.any { !it.symbol.owner.constructedClass.isSpecialAnnotation() }) {
            context.ktDiagnosticReporter.at(reportErrorsOn, containingFile).report(KtErrorsJvm.TYPEOF_ANNOTATED_TYPE)
        }
    }
    ...
    override fun reportSuspendTypeUnsupported() {
        context.ktDiagnosticReporter.at(reportErrorsOn, containingFile).report(KtErrorsJvm.TYPEOF_SUSPEND_TYPE)
    }
    ...
    override fun reportNonReifiedTypeParameterWithRecursiveBoundUnsupported(typeParameterName: Name) {
        context.ktDiagnosticReporter.at(reportErrorsOn, containingFile)
            .report(KtErrorsJvm.TYPEOF_NON_REIFIED_TYPE_PARAMETER_WITH_RECURSIVE_BOUND, typeParameterName.asString())
    }
}
