...
enum class EvaluationMode(protected val mustCheckBody: Boolean) {
    ...
    ONLY_FOLDABLE(mustCheckBody = false) {
        override fun canEvaluateFunction(function: IrFunction, context: IrCall?): Boolean {
            return function.isCompileTimePropertyAccessor() || function.isMarkedAsFoldable() || context.isFoldableNameProperty()
        }

        private fun IrFunction?.isCompileTimePropertyAccessor(): Boolean {
            val property = (this as? IrSimpleFunction)?.correspondingPropertySymbol?.owner ?: return false
            return property.isConst || (property.resolveFakeOverride() ?: property).isMarkedAsFoldable()
        }

        override fun canEvaluateEnumValue(enumEntry: IrGetEnumValue, context: IrCall?): Boolean {
            return context.isFoldableNameProperty()
        }
        ...
        private fun IrCall?.isFoldableNameProperty(): Boolean {
    ...
    protected fun IrDeclaration.isMarkedAsFoldable() = isMarkedWith(foldableAnnotation)
