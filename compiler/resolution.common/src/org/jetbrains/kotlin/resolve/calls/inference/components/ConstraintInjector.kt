...
class ConstraintInjector(
    ...
    private inner class TypeCheckerStateForConstraintInjector(
        ...
        override fun atForkPoint(block: ForkPointContext.() -> Unit): Boolean {
            if (!allowForking) {
                return super.atForkPoint(block)
            }
            ...
            val anyForkSuccessful = with(MyForkCreationContext()) {
                block()
                anyForkSuccessful
            }

            val constrainSets = stackForConstraintsSetsFromCurrentForkPoint?.popLast()?.takeIf { it.isNotEmpty() }
