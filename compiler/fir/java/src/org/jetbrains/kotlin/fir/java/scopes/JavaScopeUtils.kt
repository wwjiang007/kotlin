...
private fun FirPropertySymbol.firstOverriddenProperty(
    containingScope: FirTypeScope,
    predicate: (FirPropertySymbol) -> Boolean
): FirPropertySymbol? {
    return firstOverriddenCallable(containingScope, FirTypeScope::processOverriddenPropertiesAndSelf, predicate)
}

private fun <T : FirCallableSymbol<*>> T.firstOverriddenCallable(
    containingScope: FirTypeScope,
    predicate: (T) -> Boolean,
): T? {
    @Suppress("UNCHECKED_CAST")
    return when (this) {
        is FirNamedFunctionSymbol -> firstOverriddenFunction(containingScope, predicate as (FirNamedFunctionSymbol) -> Boolean) as T?
        is FirPropertySymbol -> firstOverriddenProperty(containingScope, predicate as (FirPropertySymbol) -> Boolean) as T?
        else -> null
    }
}
