...
fun CompilerConfiguration.setupJvmSpecificArguments(arguments: K2JVMCompilerArguments) {
    ...
    val releaseTargetArg = arguments.release
    val jvmTargetArg = arguments.jvmTarget
    if (releaseTargetArg != null) {
        val value = if (releaseTargetArg.startsWith("1.")) releaseTargetArg.substringAfter("1.").toIntOrNull() else releaseTargetArg.toIntOrNull()
        if (value == null) {
